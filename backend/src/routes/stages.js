...
router.get('/', cacheFor(60), async (req, res) => {
  const [stages] = await pool.query(
    'SELECT id, `number`, `date`, start, finish, distance_km AS distanceKm, stage_type AS stageType, started, finished FROM stages ORDER BY `number`'
  );
